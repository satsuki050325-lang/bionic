...
- [ ] `config.ts` 導入（env読み取り・validation・default値を一元化）
...
- [ ] `bionic init` 最小実装（.env.local生成・Discord Bot設定確認・Supabase接続確認）
- [ ] Engine diagnostics画面（Scheduler稼働・Discord接続・DB接続・Webhook状態）
- [ ] Appオンボーディング画面
...
- [ ] SDK event rate limit・dedupe
- [ ] payload schema versioning
- [ ] alert fingerprint見直し
- [ ] service別しきい値設定
- [ ] deployment regression閾値調整
- [ ] alert resolve flow
...
1. GitHub連携（issue/PR/CI失敗）
2. Stripe監視（payment failed / MRR change）
3. Sentry連携
4. Vercel logs深掘り（Deploy→Watch→Alert精度向上）
5. Medini以外のSDK組み込み
