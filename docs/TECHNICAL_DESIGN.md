...
```typescript
// packages/shared/src/types.ts の実装に準拠
...
承認導線（approveAction / denyAction）は現状Engine内部の `actions/service.ts` 経由で
`POST /api/actions/:id/approve | deny` として提供されており、BionicEngineService interfaceには含めていない。
retryAction も同様にPhase 2.0で正式化する予定（ROADMAP参照）。
...
> **将来（multi-tenant対応）**
> `projects` / `services` テーブルは、プロジェクト・サービスを正規化して管理する設計として予定している。
> 現在は `project_id='project_bionic'` 固定 + `service_id` を文字列で各テーブルに持つ運用で、
> まだ専用テーブルは切っていない。Phase 2.1以降で導入する。

### engine_events
外部から届いたシグナルの正規化保存。`client_event_id` はSDK側からの再送を**検知・検索するためのindex**として保持する（現状はUNIQUE制約ではなく通常index）。
将来タスクとして「`client_event_id` のUNIQUE制約化 + 重複時は409 / upsert方針の確定」を予定している。
...
### engine_alerts
- `type`: service_health / service_error / research_digest / job_failure / deployment_regression
...
- `type`: notify_discord / create_alert / run_research_digest / mark_digest_sent / retry_job / evaluate_deployment_watch
- `mode`: automatic / approval_required / manual
- `status`: pending / running / succeeded / failed / skipped / pending_approval / approved / denied / cancelled
- `approved_by` / `approved_at` / `denied_by` / `denied_at`: 承認フロー
- `input` / `result` / `error`: 実行コンテキスト

### deployments
Vercel Deployごとに1行。`watch_until` まで error eventsを集計。
- `watch_status`: pending / watching / alerted / completed / failed
- `baseline_error_count` / `current_error_count` / `error_increase_percent`
