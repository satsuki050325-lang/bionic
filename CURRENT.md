...
### 今すぐやること
Event → Alert 最小Decisionの実装
POST /api/events でeventを保存した後、
service.health.degraded / service.error.reported の場合に
engine_alertsを自動生成する処理を追加する。

### done条件
- [ ] service.health.degradedを送るとengine_alertsにレコードが生成される
- [ ] service.error.reportedを送るとengine_alertsにレコードが生成される
- [ ] 重複防止キー（fingerprint）を設計して重複alertを防ぐ
- [ ] GET /api/alerts でそのalertが取得できる
- [ ] Bionic AppのAlerts画面に表示される

---

## 確定優先順位（Phase 2）

Event → Alert 最小Decision（今ここ）
Scheduler（node-cronでweekly digest自動実行）
project_bionic / default 混在整理
App UI改善 + Recent Events表示
RLS / Security設計
Deploy→Watch→Alert（Vercel Webhook連携）
...
### 要Codex確認
- engine_alertsの重複防止fingerprint設計
...
- engine_alertsの重複防止キーをまだ設計していない
